...
import { COLORS, CONFIG, DNA_PALETTE } from './voxelConstants';

// Helper to prevent overlapping voxels
function setBlock(map: Map<string, VoxelData>, x: number, y: number, z: number, color: number, type?: number) {
    const rx = Math.round(x);
    const ry = Math.round(y);
    const rz = Math.round(z);
    const key = `${rx},${ry},${rz}`;
    map.set(key, { x: rx, y: ry, z: rz, color, type });
}
...
export const Generators = {
    PrimordialSoup: (): VoxelData[] => {
        const map = new Map<string, VoxelData>();
        const count = 400;
        const range = 10;
        
        for(let i = 0; i < count; i++) {
            // Random float positions for soup
            const x = (Math.random() - 0.5) * range * 2;
            const y = (Math.random() * range) + CONFIG.FLOOR_Y + 5;
            const z = (Math.random() - 0.5) * range * 2;
            
            // 0-3 type
            const type = Math.floor(Math.random() * 4);
            const color = DNA_PALETTE[type];
            
            // We use setBlock to ensure integer grid alignment initially, 
            // even though physics will move them off-grid
            setBlock(map, x, y, z, color, type);
        }
